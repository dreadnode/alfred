...
env var / --api-key flag for higher limits (1000 req/min).
...
# API key: set via S2_API_KEY env var or --api-key flag for higher rate limits.
...
def _api_get(url: str) -> dict[str, Any] | None:
    ...
    headers: dict[str, str] = {"User-Agent": "agentic-latex/1.0"}
    if _api_key:
...
def main() -> None:
    global _api_key
    key_help = "Semantic Scholar API key (or set S2_API_KEY env var)"
