...
class TestRenderBibliography:
    ...
    def test_builtin_style_emits_no_duplicate_bibliography_command(self):
        assert render_bibliography(MANIFEST, bib_system="builtin") == ""
...
class TestRenderAuthorBlock:
    ...
    def test_ndss_format_is_anonymous_by_default(self):
        result = render_author_block(MANIFEST, "ndss")
        assert "\\ifndssanonymous" in result
        assert "\\author{\\IEEEauthorblockN{Anonymous Submission}}" in result
        assert "Alice Smith" in result
    ...
    def test_cvpr_format(self):
        result = render_author_block(MANIFEST, "cvpr")
        assert "{\\tt\\small alice@mit.edu}" in result
        assert "\\and" in result

    def test_aaai_format(self):
        result = render_author_block(MANIFEST, "aaai")
        assert "Alice Smith\\textsuperscript{\\rm 1}" in result
        assert "\\affiliations{" in result
        assert "\\texttt{alice@mit.edu}" in result

    def test_lncs_format(self):
        result = render_author_block(MANIFEST, "lncs")
        assert "Alice Smith\\inst{1}" in result
        assert "\\authorrunning{Alice Smith and Bob Jones}" in result
        assert "\\institute{" in result

    def test_icml_format(self):
        result = render_author_block(MANIFEST, "icml")
        assert "\\twocolumn[" in result
        assert "\\icmlauthor{Alice Smith}{aff0}" in result
        assert "\\icmlaffiliation{aff0}{MIT}" in result
        assert "\\icmlcorrespondingauthor{Alice Smith}{alice@mit.edu}" in result
        assert "\\printAffiliationsAndNotice{}" in result
    ...
    def test_conference_formats_handle_empty_author_lists(self, author_format):
        result = render_author_block({"title": "Anonymous Paper"}, author_format)
        assert "Anonymous Author" in result
        assert "\n  \n" not in result
...
class TestEnsureSectionFiles:
    ...
    @pytest.mark.parametrize("slug", ["../../outside", "/tmp/outside", "nested/name"])
    def test_rejects_section_paths_outside_section_dir(self, tmp_path, slug):
        """Section slugs must never be interpreted as filesystem paths."""
        (tmp_path / "section").mkdir()
        manifest = {"sections": [{"slug": slug, "title": "Bad"}]}
        with pytest.raises(ValueError, match="Invalid section slug"):
            ensure_section_files(manifest, str(tmp_path))


class TestEnsureStyleFiles:
    def test_copies_selected_repo_style(self, tmp_path):
        manifest = {"styles": ["messageboxes"]}
        changes = ensure_style_files(manifest, str(tmp_path))
        assert (tmp_path / "styles" / "messageboxes.sty").is_file()
        assert changes == ["Copied: styles/messageboxes.sty"]

    def test_rejects_style_path(self, tmp_path):
        with pytest.raises(ValueError, match="Invalid style name"):
            ensure_style_files({"styles": ["../../outside"]}, str(tmp_path))
...
class TestSyncEndToEnd:
    ...
    def test_unknown_template_fails_without_mutation(self, tmp_project):
        yaml_content = (tmp_project / "paper.yaml").read_text()
        (tmp_project / "paper.yaml").write_text(
            yaml_content.replace("template: article", "template: missing-template")
        )
        (tmp_project / "main.tex").write_text(MAIN_TEX_WITH_MARKERS)
        original = (tmp_project / "main.tex").read_text()

        assert sync(str(tmp_project)) == 1
        assert (tmp_project / "main.tex").read_text() == original
        assert not (tmp_project / "section" / "00_abstract.tex").exists()

    def test_unknown_style_fails_before_creating_sections(self, tmp_project):
        yaml_content = (tmp_project / "paper.yaml").read_text()
        (tmp_project / "paper.yaml").write_text(
            yaml_content.replace("  - messageboxes", "  - missing-style")
        )
        (tmp_project / "main.tex").write_text(MAIN_TEX_WITH_MARKERS)
        original = (tmp_project / "main.tex").read_text()
