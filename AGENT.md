...
```bash
python3 scripts/init_template.py neurips
```

This copies the template's `main.tex` and style files to the project root, updates `paper.yaml`, and runs sync.

Run `python3 scripts/init_template.py --list` for the authoritative catalog.
It includes current NeurIPS, ICLR, ICML, CVPR, AAAI, Springer LNCS, NDSS,
IEEE, ACM, USENIX, ACL, and plain article formats. TeX Live Full is recommended
for the official conference kits.
